...
class Game:
    ...
    def tick(self):
        ...
        for player in self.players:
            ...
            if 0 < new_x < self.width - 1 or 0 < new_y < self.height - 1:  # check if player still in playing field
        ...
        if player.get_active() and new_x >= 0:
            if self.counter % 6 != 0:  # normal turn (no jump)
                for i in range(old_x - new_x):
                    if self.board[new_y][old_x - i-1] == 0:  # cell was previously empty
        ...
        if player.get_active() and new_y >= 0:
            if self.counter % 6 != 0:  # normal turn no "jump"
                for i in range(old_y - new_y):
                    if self.board[old_y - i-1][new_x] == 0:  # cell was previously empty
                        self.board[old_y - i-1][new_x] = player.get_id()
                    else:
                        player.deactivate()
                        break
            else:  # every 6th round make the player "jump"
        ...
        if player.get_active() and new_y < self.height:
            if self.counter % 6 != 0:  # normal turn no jump
                for i in range(new_y - old_y):
                    if self.board[old_y + i+1][new_x] == 0:
                        self.board[old_y + i+1][new_x] = player.get_id()
                    else:
                        player.deactivate()
                        break
            else:  # every 6th round make player jump
